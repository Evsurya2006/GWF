...
export default function PageTwo() {
  ...
  const prevArticleId = location.state?.prevArticleId;
  ...
  useEffect(() => {
    async function loadData() {
      const [art, adData] = await Promise.all([
        getRandomArticle(prevArticleId),
        getAdSettings()
      ]);
      setArticle(art);
      setAds(adData);
    ...
    <div className="min-h-screen bg-[#F8FAF8] pb-24">
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        {article ? (
          <article className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="p-8 md:p-12">
              ...
              {/* Timer Page 2 Ad (Top) */}
              {ads?.timerPage2 && (
                <div className="my-8 flex justify-center bg-gray-50 py-4 rounded-xl border border-dashed border-gray-200" dangerouslySetInnerHTML={{ __html: ads.timerPage2 }} />
              )}
        ...
        <div className="mt-12 flex justify-center">
          {timeLeft === 0 && (
            <button 
              onClick={() => navigate(`/redirect/${shortCode}`)}
              className="px-8 py-4 bg-[#1a1a1a] text-white rounded-full font-bold text-lg hover:bg-black shadow-xl flex items-center gap-2 transition-transform hover:-translate-y-1"
            >
